...
use App\Models\OurBrandSettings;
...
class OurbrandController extends Controller
{
    ...
    public function ourbrandSettings(Request $request)
    {
      
      $this->data['page_title']='Brand Settings Edit';
      $this->data['panel_title']='Brand Settings Edit';
      $encryptString= encrypt('1', Config::get('Constant.ENC_KEY'));  
      $brandSettingsId = decrypt($encryptString, Config::get('Constant.ENC_KEY'));
      $this->data['details'] = OurBrandSettings::findOrFail($brandSettingsId);
      $this->data['encryptCode'] = $encryptString;
      // echo "<pre>";
      // print_r($this->data['details']); die;
      return view('admin.ourbrand.ourbrandsettings',$this->data);
    }

    public function ourbrandSettingsUpdate(Request $request){
      try
      {
        if ($request->isMethod('POST'))
        {

              $brandSettingsId = !empty($request->encString) ? decrypt($request->encString, Config::get('Constant.ENC_KEY')) : 0;
              
              $ourBrandModelObj =  OurBrandSettings::findOrFail($brandSettingsId);
              $ourBrandModelObj->mussic_button_title_en        = trim($request->mussic_button_title_en, ' ');
              $ourBrandModelObj->download_button_title1_en         = trim($request->download_button_title1_en, ' ');
              $ourBrandModelObj->download_button_title2_en        = trim($request->download_button_title2_en, ' ');
              $ourBrandModelObj->mussic_button_title_ar       = trim($request->mussic_button_title_ar, ' ');
              $ourBrandModelObj->download_button_title1_ar         = trim($request->download_button_title1_ar, ' ');
              $ourBrandModelObj->download_button_title2_ar        = trim($request->download_button_title2_ar, ' ');
              
              $ourBrandModelObj->created_at      = Carbon::now();
              $ourBrandModelObj->updated_at      = Carbon::now();
              $save = $ourBrandModelObj->save(); 

              
              if($save){
                  return redirect()->route('admin.ourbrand.ourbrandsettings')->with('success','Our brand setings has been updated successfully.');;
