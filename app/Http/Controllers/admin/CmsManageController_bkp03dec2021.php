    ...
    public function cmsSave(CmsRequest $request){
        try
        {
        	if ($request->isMethod('POST'))
        	{
                ...
                $cmsModelObj->displayabout        = $request->displayabout;
                $cmsModelObj->display_order        = $request->display_order;

                $cmsModelObj->displayunderinvestor = $request->displayunderinvestor;

                $cmsModelObj->brochure_label_en    = trim($request->brochure_label_en, ' ');
                $cmsModelObj->brochure_label_ar    = trim($request->brochure_label_ar, ' ');

                $cmsModelObj->meta_title             = trim($request->meta_title, ' ');
                $cmsModelObj->meta_descriptions      = trim($request->meta_descriptions, ' ');
                $cmsModelObj->meta_keyword           = trim($request->meta_keyword, ' ');
                $cmsModelObj->alt_text               = trim($request->alt_text, ' ');
                $cmsModelObj->canonical              = trim($request->canonical, ' ');

                $cmsModelObj->meta_title_ar             = trim($request->meta_title_ar, ' ');
                $cmsModelObj->meta_descriptions_ar      = trim($request->meta_descriptions_ar, ' ');
                $cmsModelObj->meta_keyword_ar           = trim($request->meta_keyword_ar, ' ');
                $cmsModelObj->alt_text_ar               = trim($request->alt_text_ar, ' ');
                $cmsModelObj->canonical_ar              = trim($request->canonical_ar, ' ');

                //==========code by pk date: 25/11/2021 ============//
                //echo htmlentities($request->json_ld); die;
                //echo $request->json_ld; die; 
                $cmsModelObj->json_ld = $request->json_ld;
                $cmsModelObj->global_site_tag = $request->global_site_tag;
                //==========code by pk date: 25/11/2021 ============//
                ...
                if($request->file('brochure_en')) {
                    $files=$request->file('brochure_en');
                    $fullFileName = time().'.'.$files->getClientOriginalExtension();
                    $destinationPath = 'assets/cms/brochure/brochure_en';
                    $uploadResponse  = $files->move($destinationPath,$fullFileName);
                    $cmsModelObj->brochure_en=$fullFileName; 
                              
                }
                if($request->file('brochure_ar')) {
                    $files=$request->file('brochure_ar');
                    $fullFileName = time().'.'.$files->getClientOriginalExtension();
                    $destinationPath = 'assets/cms/brochure/brochure_ar';
                    $uploadResponse  = $files->move($destinationPath,$fullFileName);
                    $cmsModelObj->brochure_ar=$fullFileName; 
