    ...
    public function update(HomeSettingRequest $request){
        ...
        try
        {
        	if ($request->isMethod('POST'))
        	{
                ...
                $homeModelObj->meta_title = $request->meta_title;
                $homeModelObj->meta_descriptions = $request->meta_descriptions;
                $homeModelObj->meta_keyword = $request->meta_keyword;
                $homeModelObj->alt_text = $request->alt_text;
                $homeModelObj->canonical = $request->canonical;

                $homeModelObj->meta_title_ar             = trim($request->meta_title_ar, ' ');
                $homeModelObj->meta_descriptions_ar      = trim($request->meta_descriptions_ar, ' ');
                $homeModelObj->meta_keyword_ar           = trim($request->meta_keyword_ar, ' ');
                $homeModelObj->alt_text_ar               = trim($request->alt_text_ar, ' ');
                $homeModelObj->canonical_ar              = trim($request->canonical_ar, ' ');
