...
use App\Models\{ContactSeo,JoinSeo,MediacenterSeo,ServiceSeo,CommunitiesSeo,OurachievementsSeo,SearchSeo,BlogSeo};
...
class SocialController extends Controller
{
    ...
    public function pagetitleupdate(Request $request){
        try
        {
        	if ($request->isMethod('POST'))
        	{
                ...
                //------for blog title--------//
                $pTitleModelObj->blog_en                = trim($request->blog_en, ' ');
                $pTitleModelObj->blog_ar                = trim($request->blog_ar, ' ');  
                //------for blog title--------//
  ...
  public function blogSeoEdit(Request $request) {
    $this->data['page_title']='Blog Page SEO Edit';
    $this->data['panel_title']='Blog SEO Edit';
    $encryptString= encrypt('1', Config::get('Constant.ENC_KEY'));; 
    $socialId = decrypt($encryptString, Config::get('Constant.ENC_KEY')); // get user-id After Decrypt with salt key.
    $this->data['details'] = BlogSeo::findOrFail($socialId);
    // echo "<pre>";
    // print_r($this->data['details']); die;
    $this->data['encryptCode'] = $encryptString;
    //dd($this->data['details']);
    return view('admin.blog_seo.edit',$this->data); 
  }
  public function blogSeoupdate(Request $request){
    try
    {
      if ($request->isMethod('POST'))
      {
        $socialId = !empty($request->encString) ? decrypt($request->encString, Config::get('Constant.ENC_KEY')) : 0;
        //echo $socialId; die;
        $blogSeoObj =  BlogSeo::findOrFail($socialId);
        // echo "<pre>";
        // print_r($blogSeoObj); die;
        $blogSeoObj->meta_title = $request->meta_title;
        $blogSeoObj->meta_descriptions = $request->meta_descriptions;
        $blogSeoObj->meta_keyword = $request->meta_keyword;
        $blogSeoObj->alt_text = $request->alt_text;
        $blogSeoObj->canonical = $request->canonical;

        $blogSeoObj->meta_title_ar             = trim($request->meta_title_ar, ' ');
        $blogSeoObj->meta_descriptions_ar      = trim($request->meta_descriptions_ar, ' ');
        $blogSeoObj->meta_keyword_ar           = trim($request->meta_keyword_ar, ' ');
        $blogSeoObj->alt_text_ar               = trim($request->alt_text_ar, ' ');
        $blogSeoObj->canonical_ar              = trim($request->canonical_ar, ' ');
        

        $blogSeoObj->created_at      = Carbon::now();
        $blogSeoObj->updated_at      = Carbon::now();

        $save = $blogSeoObj->save(); 
        
        if($save){
            return redirect()->route('admin.social.editblogseo')->with('success','Blog page SEO has been updated successfully.');;
