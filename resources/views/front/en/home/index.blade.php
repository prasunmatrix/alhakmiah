      ...
      <div class="row justify-content-center">
        <div class="col-lg-8">
          <div class="welcome-main">
            ...
            <h2>{{$homePageSetting['title_en']}}</h2>
            <p >{!! $homePageSetting['description_en'] !!}
            </p>
...
@if($homeProject->count() > 0)
  <section class="latest-project">
    <div class="container">
    <h2>{{$homePageSetting->feature_project_heading_en}}</h2>
      <div class="row align-items-center">
        <div class="col-lg-6">
        ...
        <div class="col-lg-6">
          ...
          <h3>{{$homeProject['name']}}</h3>
          <p>{!! $homeProject->short_description_en !!}</p>
          <a class="site-link" href="{{route('en.projectdetail.detail',[$homeProject['slug_name']])}}">{{$homePageSetting->feature_project_button_text_en}}</a>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="welcome alt">
    <div class="container">
      <div class="row justify-content-center">
        <div class="col-lg-8">
          <div class="welcome-main">
            <h2 >{{$homePageGallery['title_en']}}</h2>
            <p >{!! $homePageGallery['description_en'] !!}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      @foreach($homeGallery as $data)
        <div class="item">
          <div class="prop-slide-single">
            @if (!empty($data->image))
            <a href="{{ asset('/assets/cms/gallery') }}/{{ $data->image }}" class="html5lightbox" data-group="set1">
                <img src="{{ asset('/assets/cms/gallery') }}/{{ $data->image }}" alt=""></a>
            @else
                <img src="{{ asset('/admin/upload/common/no-image-found.jpg') }}" alt="">
            @endif
...
@if(count($homeService) > 0)
  <section class="benefits">
    <div class="container">
      <h2><?php echo $socialLink[0]['service_title_en'];?></h2>
        ...
        @foreach($homeService as $data)
          <div class="col-lg-4" >
            <div class="benefits-single">
              <h3>{{$data['title_en'] }}</h3>
             <p>{!! ($data['short_description_en']) !!}</p>
              <!-- <a class="site-link gradient" href="{{ url('/') }}/service/#each-service-{{$data['title_en']}}">Read More</a> -->
            </div>
          </div>
        @endforeach
        </div>
...
@if(count($homeNews) > 0)
  <section class="architecture alt">
    <div class="container">
    <h2><?php echo $socialLink[0]['latest_news_title_en'];?></h2>
    </div>
  </section>
        ...
        <div class="col-lg-12">
          <div class="news-slide">
            <div class="owl-carousel owl-theme" id="newsslider">      
              ...
              <div class="box">
                <div class="title"><h2>{{ $data['title_en'] }}</h2></div>
                <div class="pic"><img src="{{ asset('/assets/images') }}/{{ $data->slider_1 }}" alt=""></div>
                <div class="txt">                  
                  <p> {!! ($data->short_description_en) !!}</p>
                  <a class="site-link gradient" href="{{ url('/') }}/en/news-details/{{ $data['slug_name'] }}">Read More</a>
                </div>                
              </div>
...
@endif
  <section class="architecture sec" >
    <div class="container">
      <h2><?php echo $socialLink[0]['achievement_title_en'];?></h2>
    </div>
  </section>
