...
<html lang="en">
  <head>
    ...
    <title>{{$metaDetail[$slug_name]['title_ar']}}</title>
    <meta name="description" content="{{$metaDetail[$slug_name]['meta_descriptions_ar']}}" />
    <link rel="canonical" href="{{$metaDetail[$slug_name]['canonical_ar']}}" />
    <meta name="keywords" content="{{$metaDetail[$slug_name]['meta_keyword_ar']}}">

    @if(Request::is('project-detail/*'))
      @if(!empty($projectDetail->json_ld))
      {!! html_entity_decode($projectDetail->json_ld) !!}
      @endif
      @if(!empty($projectDetail->global_site_tag))
      {!! html_entity_decode($projectDetail->global_site_tag) !!}
      @endif
    @endif
    @if(Request::is('cms/*'))
      ...
      @endif
      @if(!empty($cmsPage->global_site_tag))
      {!! html_entity_decode($cmsPage->global_site_tag) !!}
      @endif
    @endif
    ...
    <section class="header">
      <div class="container" data-aos="fade-up">
        <div class="row align-items-center">
          <div class="col-lg-2">
            <div class="logo">
              <a href="{{ url('/') }}"><img src="{{ asset('assets/images/'.$headerDetails->header_logo.'') }}" alt=""></a>
              <div class="mobClick">
                <span></span>
                <span></span>
                <span></span>
              ...
              <ul>
                ...
                ? 'active' : ''}}" href="#">‫‫من نحن</a>
                  ...
                  <?php
                      ...
                      { ?>
                        <li><a class="menu-pic {{ ($menu->slug_name==$currentSlug) ? 'active' : '' }}"  id="menu_{{$key}}" href="{{ url('/') }}/cms/{{$menu->slug_name}}">{{$menu->name_ar}}</a></li>
                      <?php
                      }
                      ?>
                    <li><a class="menu-pic {{(Request::is('ar/our-achievements') || Request::is('ar/our-achievements/*')) ? '' : ''}}" id="menu_eleven" href="{{ url('/') }}/our-achievements">{{$ourAchievementTitle->ach_title_ar}}</a></li>
                ...
                <li><a class="communitu-main-menu {{(Request::is('communities') || Request::is('communities/*') || in_array(\Request::route()->getName(), $projectRoutes)) ? 'active' : ''}}" href="{{ url('/') }}/communities" data-cid="{{ $cid }}">مجتمعاتنا</a>
                  <ul>
                    @foreach ( $projectDetails as $project )
                    
                     <li class="">
                     
                       <a class="menu-pic-community @if ($project->slug_name == $communityDetailsSlug)active @endif "  id="community_{{ $project->id }}" href="{{route('projectdetail.detail',[$project->slug_name])}}">{{ $project->name_ar }}</a>
                     </li>
                     @endforeach
                ...
                <li><a class="{{(Request::is('service') || Request::is('service/*')) ? 'active' : ''}}" href="{{ url('/') }}/service">خدماتنا</a></li>
                ...
                {{--<a class="investor-main-menu {{(Request::is('ar/investor-relations') || Request::is('ar/investor-relations/*') || $currentSlug=='board-of-directors' || $currentSlug=='executive-management' || $currentSlug=='company-committes' || $currentSlug=='governance' || $currentSlug=='the-company-article')  ? 'active' : ''}}" href="{{ url('/') }}/ar/investor-relations">علاقات المستثمرين</a>--}}
                    ...
                    <?php
                      ...
                      { ?>
                        <li><a class="menu-pic-investor {{ ($menu->slug_name==$currentSlug) ? 'active' : '' }}"  id="divmenu_{{$key}}" data-cid="{{$key}}" href="{{ url('/') }}/cms/{{$menu->slug_name}}">{{$menu->name_ar}}</a></li>
                      <?php
                      }
                      ?>
                ...
                <li><a class="{{(Request::is('media-center') || Request::is('media-center/*') || in_array(\Request::route()->getName(), $newsRoutes)) ? 'active' : ''}}" href="{{ url('/') }}/media-center">المركز الإعلامي</a></li>
                <li><a class="{{(Request::is('join-us') || Request::is('join-us/*')) ? 'active' : ''}}" href="{{ url('/') }}/join-us">انضم لنا</a></li>
                <li><a class="{{(Request::is('contact-us') || Request::is('contact-us/*')) ? 'active' : ''}}" href="{{ url('/') }}/contact-us">اتصل بنا</a></li>
